    ...
    <f7-page @page:afterin="onPageAfterIn">
        ...
        <f7-list form strong inset dividers class="margin-vertical" v-else-if="!loading">
            ...
            <f7-list-item
                class="list-item-with-header-and-title"
                link="#"
                :header="tt('Account Tag')"
                :title="account.tags && account.tags.length ? account.tags.join(', ') : tt('None')"
                @click="accountContext.showAccountTagPopup = true"
            >
                <account-tag-selection-sheet
                    :title="tt('Account Tag')"
                    :enable-filter="true"
                    :tags="accountTagOptions"
                    v-model:show="accountContext.showAccountTagPopup"
...
import AccountTagSelectionSheet from '@/components/mobile/AccountTagSelectionSheet.vue';
...
const accountTagOptions = computed(() => accountTagsStore.allAccountTags);
